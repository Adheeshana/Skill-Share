...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled 
        ? 'bg-white/90 backdrop-blur-lg text-teal-800 shadow-lg' 
        : 'bg-gradient-to-r from-teal-800/90 via-teal-600/90 to-emerald-700/90 backdrop-blur-md text-white shadow-md'
    }`}>
      {/* Animated top border with enhanced shimmer effect */}
      <div className="h-1.5 w-full bg-gradient-to-r from-amber-400 via-teal-500 to-emerald-600 background-animate relative overflow-hidden">
          ...
          <div className="flex items-center">
            <Link className="flex items-center font-bold text-xl group logo-container relative" to="/">
              ...
              <div className={`flex items-center justify-center h-10 w-10 rounded-full mr-2 ${
                scrolled ? 'bg-gradient-to-br from-teal-500 to-emerald-700 text-white' : 'bg-white text-teal-600'
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-yellow-300/0 to-amber-500/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500 animate-pulse-slow"></div>
              ...
              <span className="relative overflow-hidden flex">
                <span className="transition-all duration-300 mr-0.5 group-hover:text-amber-500 inline-block transform group-hover:translate-y-[-2px]">Skill</span>
                <span className={`font-light ${scrolled ? 'text-teal-600' : 'text-amber-400'} inline-block transform group-hover:translate-y-[-2px]`}>Share</span>
                {/* Enhanced floating decorative elements */}
                <span className="absolute -top-1 -right-1 h-1 w-1 bg-amber-300 rounded-full opacity-0 group-hover:opacity-100 animate-float transition-opacity" style={{animationDelay: "0.2s"}}></span>
                ...
                <span className="absolute bottom-2 right-1 h-1 w-1 bg-teal-300 rounded-full opacity-0 group-hover:opacity-100 animate-float transition-opacity" style={{animationDelay: "0.4s"}}></span>
                {/* Decorative underline that animates on hover */}
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-amber-400 to-teal-400 group-hover:w-full transition-all duration-300"></span>
              </span>
            </Link>
          </div>
            ...
            <button 
              ...
              className={`inline-flex items-center justify-center p-2 rounded-md transition-all duration-300
                ${scrolled 
                  ...
                  : 'text-white hover:bg-teal-700/40 hover:backdrop-blur-sm'
                } focus:outline-none focus:ring-2 focus:ring-inset focus:ring-teal-300`}
            ...
            <div className="flex space-x-1">
              ...
              <button 
                ...
                className={`group p-2 rounded-md text-sm font-medium transition-all duration-300 ${
                  scrolled 
                    ...
                    : 'text-white/80 hover:bg-teal-700/40 hover:text-white hover:backdrop-blur-sm'
                }`}
              >
                ...
                <div className="absolute top-full left-0 mt-1 w-full bg-white shadow-lg rounded-b-xl p-3 transform animate-fadeDown">
                  <form onSubmit={handleSearch} className="flex items-center">
                    <div className="relative flex-grow">
                      <input
                        type="search"
                        className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-l-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400"
                    ...
                    <button 
                      type="submit" 
                      className="bg-gradient-to-r from-teal-600 to-emerald-700 hover:from-teal-700 hover:to-emerald-800 text-white px-4 py-2 rounded-r-lg transition-all shadow-md"
                    >
                      Search
                    </button>
                  </form>
              ...
              <Link 
                className={`group px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 ${
                  activeLink === "home" 
                    ? (scrolled ? 'bg-teal-50 text-teal-700' : 'bg-white/10 text-white') 
                    : (scrolled 
              ...
              >
                <span className="flex items-center relative">
                  ...
                  {activeLink === "home" && (
                    <span className={`absolute -bottom-1 left-0 h-0.5 w-full ${scrolled ? 'bg-teal-500' : 'bg-amber-300'}`}></span>
                  )}
                  <span className={`absolute -bottom-1 left-0 h-0.5 w-0 ${scrolled ? 'bg-teal-500' : 'bg-amber-300'} group-hover:w-full transition-all duration-300`}></span>
                </span>
              </Link>
              ...
              <Link 
                className={`group relative px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 ${
                  activeLink === "create-path" 
                    ...
                    : (scrolled 
                      ? 'bg-gradient-to-r from-teal-600 to-emerald-700 hover:from-teal-700 hover:to-emerald-800 text-white shadow-md hover:shadow-lg' 
              ...
              >
                <span className="flex items-center relative z-10">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1.5 text-amber-300 group-hover:scale-110 transition-all duration-300 group-hover:animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <span className="absolute -right-1 -top-1 h-2 w-2 rounded-full bg-amber-400 opacity-0 group-hover:opacity-100 animate-ping"></span>
                <span className="absolute -left-1 -bottom-1 h-2 w-2 rounded-full bg-teal-400 opacity-0 group-hover:opacity-80 animate-ping" style={{animationDelay: "0.5s"}}></span>
              </Link>
              ...
              {currentUser && (
                <div className="relative">
                  <button className={`p-2 rounded-full transition-all duration-300 ${
                    ...
                    {notificationCount > 0 && (
                      <span className="absolute top-1 right-1 bg-amber-500 text-white text-xs w-4 h-4 flex items-center justify-center rounded-full">{notificationCount}</span>
                    )}
                  </button>
                </div>
              )}
              ...
              {currentUser ? (
                <div className="relative" ref={dropdownRef}>
                  <button 
                    ...
                    ${scrolled 
                      ? 'bg-teal-50 text-teal-800 hover:bg-teal-100 hover:shadow-md' 
                  ...
                  >
                    {currentUser.profilePicture ? (
                      ...
                      <div className={`rounded-full flex items-center justify-center mr-2 h-7 w-7 text-xs font-bold shadow-sm border-2
                        ${scrolled ? 'bg-teal-600 text-white border-teal-200' : 'bg-white text-teal-600 border-white/30'}`}
                      >
                        ...
                        <div className="flex items-center">
                          {currentUser.profilePicture ? (
                            <img 
                              src={currentUser.profilePicture} 
                              alt={currentUser.username} 
                              className="rounded-full mr-3 h-10 w-10 object-cover border-2 border-teal-100 shadow-sm"
                            />
                          ) : (
                            <div className="rounded-full flex items-center justify-center mr-3 h-10 w-10 text-sm font-bold shadow-sm border-2 bg-teal-600 text-white border-teal-200">
                              {currentUser?.username ? currentUser.username.charAt(0).toUpperCase() : 
                              currentUser?.name ? currentUser.name.charAt(0).toUpperCase() : 
                              currentUser?.email ? currentUser.email.charAt(0).toUpperCase() : '?'}
                            </div>
                          ...
                          <div>
                            ...
                            <p className="text-sm font-bold text-teal-700 truncate">{currentUser?.username || currentUser?.email}</p>
                          </div>
                        </div>
                      </div>
                      <div className="py-1">
                        <Link 
                          to="/profile" 
                          className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-teal-50 transition-colors"
                          onClick={() => setShowDropdown(false)}
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      >
        <div className="px-2 pt-2 pb-3 space-y-1">
          <Link 
            to="/"
            className="flex items-center text-teal-700 hover:bg-teal-50 block px-3 py-2 rounded-md text-base font-medium transition-all"
          ...
          {currentUser ? (
            ...
            <>
              ...
              <Link 
                to="/login" 
                className="flex items-center text-teal-700 hover:bg-teal-50 px-3 py-2.5 rounded-md text-base font-medium transition-all duration-300"
                onClick={() => setNavbarOpen(false)}
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
